...
classdef GrnProteinOdeConverter < synnetgen.extension.Extension
    ...
    methods (Static)
        function odes = run(network, varargin)
            ...
            nodes = repmat(struct('id', '', 'label', ''), 0, 2);
            parameters = repmat(struct('id', '', 'label', ''), 0, 1);
            differentials = cell(numel(network.nodes), 2);
            for iNode = 1:numel(network.nodes)
                nodeId = network.nodes(iNode).id;
                nodes = [
                    nodes
                    struct('id', sprintf('r_%s', nodeId), 'label', sprintf('RNA %s', nodeId)) struct('id', sprintf('p_%s', nodeId), 'label', sprintf('Protein %s', nodeId))
                    ];
                parameters = [
                    parameters
                    ...
                    struct('id', sprintf('V_max_p_%s', nodeId), 'label', sprintf('Maximum translation rate of gene %s', nodeId))
                    struct('id', sprintf('tau_p_%s', nodeId), 'label', sprintf('Half-life of protein %s', nodeId))
                    ];
                ...
                for i = 1:numel(tt)
                    ...
                    for j = 1:nRegs
                        if regVals(j)
                            ...
                            f_num_i = [f_num_i
                                sprintf('(p_%s/K_r_%s)^n_r_%s', regId, regId, regId)
                                ];
                            f_denom_i = [f_denom_i
                                sprintf('(p_%s/K_r_%s)^n_r_%s', regId, regId, regId)
                                ];
                ...
                if isempty(tt)
                    differentials{iNode, 1} = sprintf('V_max_r_%s - %.4f/tau_r_%s * r_%s', ...
                        nodeId, log(2), nodeId, nodeId);
                else
                    differentials{iNode, 1} = sprintf('V_max_r_%s * (%s)/(%s) - %.4f/tau_r_%s * r_%s', ...
                        nodeId, strjoin(f_num, ' + '), strjoin(f_denom, ' + '), log(2), nodeId, nodeId);
                end
                
                differentials{iNode, 2} = sprintf('V_max_p_%s * r_%s - %.4f/tau_p_%s * p_%s', ...
                    nodeId, nodeId, log(2), nodeId, nodeId);
            end
